...
export default async function AdminJobsPage({ searchParams }: { searchParams: Promise<{ created?: string }> }) {
      ...
      <div className="mt-7 overflow-hidden rounded-2xl border border-[#e8d8c8] bg-[#fdfaf6]">
        {jobs.length === 0 ? (
          ...
          <ul className="divide-y divide-[#e8d8c8]" role="list">
            {jobs.map((job) => (
              <li key={job.id} className="grid gap-4 px-5 py-5 sm:grid-cols-[1fr_auto] sm:items-center sm:px-6">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="font-semibold text-[#3D3027]">{job.title}</h2>
                    <span className="rounded-full bg-[#dce8dc] px-2.5 py-1 text-xs font-semibold capitalize text-[#476047]">{job.status}</span>
                  </div>
                  <p className="mt-1 text-xs text-[#9a7a63]">{job.id} · {job.cities.join(", ")} · {job.languages.join(", ")}</p>
                </div>
                <time dateTime={job.createdAt.toISOString()} className="text-xs text-[#9a7a63]">{new Intl.DateTimeFormat("en-SE", { dateStyle: "medium" }).format(job.createdAt)}</time>
              </li>
            ))}
          </ul>
        )}
