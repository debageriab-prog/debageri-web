...
export default async function CandidatesPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const [applications, templates, emailSettings] = await Promise.all([getApplications(status), getEmailTemplates(), getEmailSettings()]);
  const emailConfigured = Boolean(emailSettings.host && emailSettings.fromEmail && emailSettings.hasPassword);
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10 md:py-14">
    ...
    {!emailConfigured && <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#e8d8c8] bg-[#f0e8dc] px-5 py-4"><p className="text-sm text-[#5a4535]"><span className="font-semibold">Candidate email is not configured.</span> Add your SMTP connection to send status updates.</p><Link href="/admin/settings/email" className="rounded-full bg-[#3D3027] px-4 py-2 text-xs font-semibold text-[#F7F2EA]">Configure email</Link></div>}
    ...
    <div className="mt-6 space-y-4">{applications.length === 0 ? <div className="rounded-2xl border border-[#e8d8c8] bg-[#fdfaf6] px-6 py-20 text-center"><p className="font-semibold">No candidates here.</p><p className="mt-2 text-sm text-[#9a7a63]">New applications will appear here.</p></div> : applications.map((application) => <article key={application.id} className="rounded-2xl border border-[#e8d8c8] bg-[#fdfaf6] p-6">
      <div className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-start">
        ...
        <div className="flex flex-col gap-3"><a href={`/api/admin/applications/${application.id}/resume`} target="_blank" rel="noopener noreferrer" className="rounded-lg border border-[#c4a98e] px-4 py-2.5 text-center text-sm font-semibold text-[#5a4535]">Open {application.resumeFileName} ↗</a><CandidateStatusEditor application={application} templates={templates} emailConfigured={emailConfigured} /><DeleteCandidateButton candidateId={application.id} candidateName={`${application.firstName} ${application.lastName}`} /></div>
      </div>
    </article>)}</div>
  </main>;
}
