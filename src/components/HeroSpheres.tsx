...
export function HeroSpheres() {
  ...
  useEffect(() => {
    ...
    const centres = HERO_SPHERES.map(() => ({ x: 0, y: 0, visible: false }));
    const offsets = HERO_SPHERES.map(() => ({ x: 0, y: 0 }));
    ...
    const tick = () => {
      ...
      for (let i = 0; i < HERO_SPHERES.length; i += 1) {
        const el = wrapRefs.current[i];
        const centre = centres[i];
        const offset = offsets[i];
        if (!el || !centre.visible) continue;

        const { size, strength } = HERO_SPHERES[i];
  ...
  return (
    ...
    >
      {HERO_SPHERES.map(({ wrap, sphere }, index) => (
        <span
          key={wrap}
          ref={(el) => {
