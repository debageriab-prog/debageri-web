...
function encryptionKey(): Buffer {
  ...
  if (key.length !== 32)
    throw new Error(
      "EMAIL_SETTINGS_ENCRYPTION_KEY must be a base64-encoded 32-byte key.",
    );
  return key;
}
...
function decrypt(value: string): string {
  ...
  if (version !== "v1" || !iv || !tag || !ciphertext)
    throw new Error("Stored SMTP password is invalid.");
...
export async function getEmailSettings(): Promise<EmailSettings> {
  ...
  return {
    ...
    fromName:
      typeof data?.fromName === "string" ? data.fromName : "Debageri AB",
    fromEmail: typeof data?.fromEmail === "string" ? data.fromEmail : "",
    replyTo: typeof data?.replyTo === "string" ? data.replyTo : "",
    adminNotificationEmail:
      typeof data?.adminNotificationEmail === "string"
        ? data.adminNotificationEmail
        : "",
    hasPassword:
      typeof data?.passwordEncrypted === "string" &&
      Boolean(data.passwordEncrypted),
  };
}
...
export async function getSmtpCredentials() {
  ...
  if (!settings.host || !settings.fromEmail || !data?.passwordEncrypted)
...
export async function saveEmailSettings(
  settings: Omit<EmailSettings, "hasPassword">,
  password: string,
  adminUid: string,
) {
...
export async function getEmailTemplates(): Promise<EmailTemplate[]> {
  ...
  return DEFAULT_EMAIL_TEMPLATES.map((template) => {
    ...
    if (!value || typeof value !== "object")
      return { ...template, body: asHtml(template.body) };
    ...
    return {
      ...
      subject:
        typeof candidate.subject === "string"
          ...
          : template.subject,
      body: asHtml(
        typeof candidate.body === "string" ? candidate.body : template.body,
      ),
    };
  });
}
